...
class Trip(Base):
    __tablename__ = 'trip'
    __table_args__ = {'schema': 'new_lab_3'}

    id = Column(Integer, primary_key=True, autoincrement=True)
    start_time = Column(String(45), nullable=False)
    end_time = Column(String(45), nullable=False)
    price = Column(Float, nullable=False)
    user_id = Column(Integer, ForeignKey('new_lab_3.user.id'), nullable=False, index=True)
    driver_id = Column(Integer, ForeignKey('new_lab_3.driver.id'), nullable=False, index=True)
